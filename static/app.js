...
const MODE_CONFIG = {
    classic: { name: 'Classic', icon: '🧠', description: 'Repeat the pattern shown' },
    speedrun: { name: 'Speed Run', icon: '⚡', description: 'Race against time' },
    endless: { name: 'Endless', icon: '♾️', description: 'Pattern grows each round' },
    simon: { name: 'Simon Says', icon: '🎯', description: 'Repeat the exact sequence' }
};
...
function cacheElements() {
    ...
    elements.backFromLeaderboardBtn = document.getElementById('backFromLeaderboardBtn');
...
function setupEventListeners() {
    ...
    elements.viewLeaderboardBtn.addEventListener('click', () => showScreen('leaderboard'));
    elements.backToStartBtn.addEventListener('click', confirmExit);
    elements.backFromLeaderboardBtn.addEventListener('click', () => showScreen('start'));
...
function attemptReconnect() {
    if (gameState.reconnectAttempts < RECONNECT_MAX_ATTEMPTS) {
        ...
        const delay = Math.min(RECONNECT_BASE_DELAY * Math.pow(2, gameState.reconnectAttempts - 1), 30000);
        console.log(`🔄 Reconnecting in ${delay}ms (${gameState.reconnectAttempts}/${RECONNECT_MAX_ATTEMPTS})`);
...
function handleTileToggled(data) {
    ...
    updateSelectionCounter(selected_tiles?.length || gameState.selectedTiles.size, expected_count);
}
...
function handleGameOver(data) {
    ...
    elements.finalLevel.textContent = LEVEL_CONFIG[gameState.level]?.label || gameState.level;
    
    // Set title
    if (gameState.score >= 100) {
        elements.gameOverTitle.innerHTML = '<span class="title-icon">🏆</span> INCREDIBLE!';
    } else if (gameState.score >= 50) {
        elements.gameOverTitle.innerHTML = '<span class="title-icon">⭐</span> GREAT JOB!';
    } else if (gameState.score > 0) {
        elements.gameOverTitle.innerHTML = '<span class="title-icon">👍</span> NICE TRY!';
    } else {
        elements.gameOverTitle.innerHTML = '<span class="title-icon">🎮</span> GAME OVER';
    }
...
function updateConnectionUI() {
    ...
    if (gameState.currentScreen === 'game' && !gameState.isPlaying) {
        if (gameState.masterConnected && connected > 0) {
            setMessage('🎮', `${connected}/${TOTAL_TILES} tiles online! Press START to begin!`);
...
async function startGame() {
    ...
    try {
        const res = await fetch(`/api/leaderboard/check-name?name=${encodeURIComponent(teamName)}`);
    ...
    if (elements.currentLevel) elements.currentLevel.textContent = LEVEL_CONFIG[gameState.level]?.label || gameState.level;
    if (elements.currentMode) elements.currentMode.textContent = MODE_CONFIG[gameState.mode]?.name || gameState.mode;
    updateGameUI();
...
async function loadLeaderboard() {
    try {
        ...
        if (elements.leaderboardPreview) {
            elements.leaderboardPreview.innerHTML = '<p class="empty-leaderboard">Unable to load</p>';
        }
    }
}
...
async function loadFullLeaderboard(level = 'all') {
    if (elements.leaderboardFull) {
        elements.leaderboardFull.innerHTML = '<div class="loading-spinner"><div class="spinner"></div></div>';
    }
    ...
    try {
        ...
        if (elements.leaderboardFull) {
            elements.leaderboardFull.innerHTML = '<p class="empty-leaderboard">Unable to load</p>';
        }
    }
}
...
function renderLeaderboard(container, entries) {
    ...
    if (!entries || entries.length === 0) {
        container.innerHTML = '<p class="empty-leaderboard">No scores yet! Be the first!</p>';
        return;
    }
    ...
    container.innerHTML = sorted.map((entry, i) => {
        ...
        const itemClass = rank === 1 ? 'gold' : rank === 2 ? 'silver' : rank === 3 ? 'bronze' : '';
